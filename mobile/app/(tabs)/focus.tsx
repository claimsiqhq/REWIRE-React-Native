...
import { Text, H1, Card, Button, Badge } from "@/src/components/ui";
...
export default function FocusScreen() {
  ...
  const getNextPhase = useCallback(
    (current: BreathPhase): { phase: BreathPhase; duration: number } | null => {
  ...
  return (
    <SafeAreaView className="flex-1 bg-background" edges={["top"]}>
        ...
        <View className="px-4 gap-2 mb-6">
          {TECHNIQUES.map((technique) => (
            <Card
              key={technique.id}
              className={`p-3 ${
                selectedTechnique.id === technique.id ? "border-2 border-accent" : ""
              }`}
      ...
      <View className="flex-1 items-center justify-center">
        <Animated.View
          className="w-[280] h-[280] rounded-full border-[3px] items-center justify-center"
          style={{
      ...
      <View className="px-4 py-6 items-center">
        {isActive ? (
          <Button variant="destructive" size="lg" onPress={stopBreathing} className="min-w-[200]">
            Stop
          </Button>
        ) : (
          <Button
